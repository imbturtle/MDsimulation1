...
void CalAveQW(int,double *, double *, double***, double***, double *, double *, double *, double *, double *, double *, double **, double **, double **, double **, double *, int **);

void Crystalcate(int, int **, double **, double **, double *, double *, double *);
#define pi M_PI
/***********************************************************************************************************
***********************************************************************************************************/
...
void CalSPBOP(int N, int **Nblist, double **Rq4, double **Iq4, double **Rq6, double **Iq6\
	...
	for(i=0;i<N;i++){
		...
		Si[i] /= 12.0;
//		fprintf(fpwqc,"%d %d %lf\n",n,i,qc);
}}
/***********************************************************************************************************
***********************************************************************************************************/
void CalAveQW(int i,double *w4top,double *w6top,double ***F6,double ***F4,double *Aveq4mR,double *Aveq4mI,double *Aveq4sq,double *Aveq6mR,double *Aveq6mI,double *Aveq6sq,double **Rq4,double **Iq4,double **Rq6,double **Iq6,double *LocNb,int **Nblist){
	int k,m,m1,m2,m3;
	for(m=0;m<13;m++){
		if(m<9){
			Aveq4mR[m] = Rq4[i][m]/LocNb[i];
			Aveq4mI[m] = Iq4[i][m]/LocNb[i];
			for(k=0;k<20;k++){
				if(Nblist[i][k] < 0) break;		
				Aveq4mR[m] += Rq4[Nblist[i][k]][m]/LocNb[Nblist[i][k]];
				Aveq4mI[m] += Iq4[Nblist[i][k]][m]/LocNb[Nblist[i][k]];
		}}
		Aveq6mR[m] = Rq6[i][m]/LocNb[i];
		Aveq6mI[m] = Iq6[i][m]/LocNb[i];
		for(k=0;k<20;k++){
			if(Nblist[i][k] < 0) break;				
			Aveq6mR[m] += Rq6[Nblist[i][k]][m]/LocNb[Nblist[i][k]];
			Aveq6mI[m] += Iq6[Nblist[i][k]][m]/LocNb[Nblist[i][k]];
	}}
	Aveq4sq[0] = Aveq4mR[0]*Aveq4mR[0]+Aveq4mI[0]*Aveq4mI[0]+Aveq4mR[1]*Aveq4mR[1]+Aveq4mI[1]*Aveq4mI[1]\
		+Aveq4mR[2]*Aveq4mR[2]+Aveq4mI[2]*Aveq4mI[2]+Aveq4mR[3]*Aveq4mR[3]+Aveq4mI[3]*Aveq4mI[3]\
		+Aveq4mR[4]*Aveq4mR[4]+Aveq4mI[4]*Aveq4mI[4]+Aveq4mR[5]*Aveq4mR[5]+Aveq4mI[5]*Aveq4mI[5]\
		+Aveq4mR[6]*Aveq4mR[6]+Aveq4mI[6]*Aveq4mI[6]+Aveq4mR[7]*Aveq4mR[7]+Aveq4mI[7]*Aveq4mI[7]\
		+Aveq4mR[8]*Aveq4mR[8]+Aveq4mI[8]*Aveq4mI[8];
	Aveq6sq[0] = Aveq6mR[0]*Aveq6mR[0]+Aveq6mI[0]*Aveq6mI[0]+Aveq6mR[1]*Aveq6mR[1]+Aveq6mI[1]*Aveq6mI[1]\
		+Aveq6mR[2]*Aveq6mR[2]+Aveq6mI[2]*Aveq6mI[2]+Aveq6mR[3]*Aveq6mR[3]+Aveq6mI[3]*Aveq6mI[3]\
		+Aveq6mR[4]*Aveq6mR[4]+Aveq6mI[4]*Aveq6mI[4]+Aveq6mR[5]*Aveq6mR[5]+Aveq6mI[5]*Aveq6mI[5]\
		+Aveq6mR[6]*Aveq6mR[6]+Aveq6mI[6]*Aveq6mI[6]+Aveq6mR[7]*Aveq6mR[7]+Aveq6mI[7]*Aveq6mI[7]\
		+Aveq6mR[8]*Aveq6mR[8]+Aveq6mI[8]*Aveq6mI[8]+Aveq6mR[9]*Aveq6mR[9]+Aveq6mI[9]*Aveq6mI[9]\
		+Aveq6mR[10]*Aveq6mR[10]+Aveq6mI[10]*Aveq6mI[10]+Aveq6mR[11]*Aveq6mR[11]+Aveq6mI[11]*Aveq6mI[11]
		+Aveq6mR[12]*Aveq6mR[12]+Aveq6mI[12]*Aveq6mI[12];
	w4top[0] = 0.0;
	w6top[0] = 0.0;
	for(m1=0;m1<13;m1++)for(m2=0;m2<13;m2++)for(m3=0;m3<13;m3++){
		if(m1+m2+m3==12 && m1<9 && m2<9 && m3<9){
		w4top[0] += F4[m1][m2][m3] * (Aveq4mR[m1]*Aveq4mR[m2]*Aveq4mR[m3] -\
...
//		w4topi += F4[m1][m2][m3] * (-Aveq4mR[m1]*Aveq4mR[m2]*Aveq4mI[m3] + \
		...
		if(m1+m2+m3==18){
			w6top[0] += F6[m1][m2][m3] * (Aveq6mR[m1]*Aveq6mR[m2]*Aveq6mR[m3] - \
...
/***********************************************************************************************************
***********************************************************************************************************/
void Reset(int N,int **Nblist,double **Nbdist,double **Rq4 ,double **Iq4 ,double **Rq6,double **Iq6\
